...
public class AABB3
{
	...
	//获取AABB所有顶点坐标的方法
	public Vector3f3D[] getAllCorners(){
		Vector3f3D[] result = new Vector3f3D[8];
		for(int i=0; i<8; i++){
	...
	//获取AABB第i个顶点坐标的方法
	public Vector3f3D getCorner(int i){		
		if(i<0||i>7){//检查i是否合法
			return null;
		}
		return new Vector3f3D(
				((i & 1) == 0) ? max.x : min.x,
				((i & 2) == 0) ? max.y : min.y, 
				((i & 4) == 0) ? max.z : min.z
				);
	...
	{
		//获取所有顶点的坐标
		Vector3f3D[] va = this.getAllCorners();
		//用于存放仿射变换后的顶点数组
	...
	public Vector3f3D getSize(){//获取对角线向量
		return max.minus(min);
	}
	//获取包围盒的中心点坐标的方法
	public Vector3f3D getCenter(){
	...
	public float rayIntersect(
			Vector3f3D rayStart,//射线起点
			Vector3f3D rayDir,//射线长度和方向
			Vector3f3D returnNormal//可选的，相交点处法向量
			){
		//如果未相交则返回这个大数
